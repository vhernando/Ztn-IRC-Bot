...
def _update(oldobj, newobj):
    ...
    """
    #print "update: %s --> %s" % (oldobj, newobj)
    if oldobj is newobj:
    ...
    if isinstance(newobj, types.ClassType):
        print "update class: %s --> %s" % (oldobj, newobj)
        return _update_class(oldobj, newobj)
    if isinstance(newobj, types.FunctionType):
        print "update funct: %s --> %s" % (oldobj, newobj)        
        return _update_function(oldobj, newobj)
    if isinstance(newobj, types.MethodType):
        print "update method: %s --> %s" % (oldobj, newobj)
...
def _update_class(oldclass, newclass):
    ...
    for name in oldnames & newnames:
        if name is not "__dict__" and name is not "__doc__":
